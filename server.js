...
app.post('/api/admin/qualifications',guardianRequireAdmin('settings.edit'),(req,res)=>{if(!Array.isArray(req.body?.qualifications))return res.status(400).json({ok:false,error:'Qualifications required'});guardianQualifications=req.body.qualifications.map((q,i)=>({id:String(q.id||`qual-${Date.now()}-${i}`),name:String(q.name||'').trim(),serviceId:String(q.serviceId||'all'),category:String(q.category||'Qualification'),description:String(q.description||''),validityMonths:Number(q.validityMonths||0),requires:Array.isArray(q.requires)?q.requires:[],discordRoleId:String(q.discordRoleId||''),syncDiscordRole:q.syncDiscordRole===true,active:q.active!==false})).filter(q=>q.name);guardianWriteJson(guardianQualificationsFile,guardianQualifications);guardianAdminAuditLog(req.guardianAdmin.username,'QUALIFICATION_CATALOG_SAVED',{count:guardianQualifications.length});res.json({ok:true,qualifications:guardianQualifications})});
app.patch('/api/admin/qualifications/:id',guardianRequireAdmin('settings.edit'),(req,res)=>{const id=String(req.params.id||'');const i=guardianQualifications.findIndex(q=>String(q.id)===id);if(i<0)return res.status(404).json({ok:false,error:'Qualification not found'});const current=guardianQualifications[i];const b=req.body||{};const updated={...current,name:b.name!==undefined?String(b.name).trim():current.name,serviceId:b.serviceId!==undefined?String(b.serviceId||'all'):current.serviceId,category:b.category!==undefined?String(b.category||'Qualification'):current.category,description:b.description!==undefined?String(b.description||''):current.description,validityMonths:b.validityMonths!==undefined?Number(b.validityMonths||0):Number(current.validityMonths||0),requires:b.requires!==undefined?(Array.isArray(b.requires)?b.requires:[]):current.requires,discordRoleId:b.discordRoleId!==undefined?String(b.discordRoleId||'').trim():String(current.discordRoleId||''),syncDiscordRole:b.syncDiscordRole!==undefined?(b.syncDiscordRole===true):current.syncDiscordRole,active:b.active!==undefined?(b.active!==false):current.active};if(!updated.name)return res.status(400).json({ok:false,error:'Qualification name required'});if(updated.syncDiscordRole&&!updated.discordRoleId)return res.status(400).json({ok:false,error:'A Discord role is required when auto-sync is enabled'});if(updated.discordRoleId&&!/^\d{15,22}$/.test(updated.discordRoleId))return res.status(400).json({ok:false,error:'Discord Role ID must be numeric'});guardianQualifications[i]=updated;guardianWriteJson(guardianQualificationsFile,guardianQualifications);guardianAdminAuditLog(req.guardianAdmin.username,'QUALIFICATION_UPDATED',{qualificationId:id,discordRoleId:updated.discordRoleId||null,syncDiscordRole:updated.syncDiscordRole});res.json({ok:true,qualification:updated,qualifications:guardianQualifications})});
async function guardianQualificationDiscordRole(user,q,add=true){
  const guildId=String(guardianDiscord.guildId||'').trim(),discordUserId=String(user?.discordUserId||'').trim(),roleId=String(q?.discordRoleId||'').trim();
  if(!q?.syncDiscordRole||!guildId||!discordUserId||!roleId)return {ok:true,skipped:true};
  return guardianDiscordRequest(add?'PUT':'DELETE',`/guilds/${encodeURIComponent(guildId)}/members/${encodeURIComponent(discordUserId)}/roles/${encodeURIComponent(roleId)}`);
}
app.post('/api/admin/users/:username/qualification-award',guardianRequireAdmin('settings.edit'),async(req,res)=>{const u=guardianAdminUsers.get(String(req.params.username||''));if(!u)return res.status(404).json({ok:false,error:'User not found'});const qid=String(req.body?.qualificationId||''),q=guardianQualifications.find(x=>x.id===qid);if(!q)return res.status(404).json({ok:false,error:'Qualification not found'});u.qualifications=Array.isArray(u.qualifications)?u.qualifications:[];u.qualifications=u.qualifications.filter(x=>(typeof x==='string'?x:x.id||x.name)!==qid && (typeof x==='string'?x:x.name)!==q.name);const months=Number(req.body?.validityMonths??q.validityMonths??0),awardedAt=new Date(),expiresAt=months>0?new Date(new Date(awardedAt).setMonth(awardedAt.getMonth()+months)).toISOString():'';u.qualifications.push({id:q.id,name:q.name,serviceId:q.serviceId,awardedAt:awardedAt.toISOString(),awardedBy:req.guardianAdmin.username,expiresAt,discordRoleId:q.discordRoleId||''});guardianSaveUsers();const discord=await guardianQualificationDiscordRole(u,q,true);guardianAdminAuditLog(req.guardianAdmin.username,'QUALIFICATION_AWARDED',{username:u.username,qualification:q.name,discordRoleSynced:!!discord.ok&&!discord.skipped});res.json({ok:true,user:guardianUserPublicProfile(u),discord})});
app.post('/api/admin/users/:username/qualification-revoke',guardianRequireAdmin('settings.edit'),async(req,res)=>{const u=guardianAdminUsers.get(String(req.params.username||''));if(!u)return res.status(404).json({ok:false,error:'User not found'});const key=String(req.body?.qualificationId||req.body?.name||'');const q=guardianQualifications.find(x=>x.id===key||x.name===key);u.qualifications=(u.qualifications||[]).filter(x=>{const id=typeof x==='string'?x:x.id||'',name=typeof x==='string'?x:x.name||'';return id!==key&&name!==key});guardianSaveUsers();const discord=q?await guardianQualificationDiscordRole(u,q,false):{ok:true,skipped:true};guardianAdminAuditLog(req.guardianAdmin.username,'QUALIFICATION_REVOKED',{username:u.username,key,discordRoleSynced:!!discord.ok&&!discord.skipped});res.json({ok:true,user:guardianUserPublicProfile(u),discord})});
...
app.get('/api/admin/discord/roles',guardianRequireAdmin('settings.view'),async(_req,res)=>{const guildId=String(guardianDiscord.guildId||'').trim();if(!guildId)return res.json({ok:true,roles:[]});const r=await guardianDiscordRequest('GET',`/guilds/${guildId}/roles`);if(!r.ok)return res.status(400).json({ok:false,error:r.error||'Unable to load Discord roles'});res.json({ok:true,roles:(r.data||[]).filter(x=>x.name!=='@everyone'&&x.managed!==true).map(x=>({id:x.id,name:x.name,color:x.color,position:x.position})).sort((a,b)=>b.position-a.position)})});
...
app.post('/api/admin/training',guardianRequireAdmin('settings.edit'),async(req,res)=>{const incoming=req.body?.session;if(!incoming)return res.status(400).json({ok:false,error:'Session required'});let t=incoming.id?guardianTraining.find(x=>x.id===incoming.id):null;if(!t){t={id:crypto.randomUUID(),bookings:[],createdAt:new Date().toISOString()};guardianTraining.unshift(t)}Object.assign(t,{title:String(incoming.title||'Training Session'),serviceId:String(incoming.serviceId||'police'),startsAt:String(incoming.startsAt||''),endsAt:String(incoming.endsAt||''),location:String(incoming.location||''),instructor:String(incoming.instructor||''),description:String(incoming.description||''),qualificationAwarded:String(incoming.qualificationAwarded||''),qualificationId:String(incoming.qualificationId||''),prerequisites:Array.isArray(incoming.prerequisites)?incoming.prerequisites:[],maxSlots:Number(incoming.maxSlots||0),bookingClosesAt:String(incoming.bookingClosesAt||''),bookingManuallyClosed:incoming.bookingManuallyClosed===true,discordChannelId:String(incoming.discordChannelId||''),documents:Array.isArray(incoming.documents)?incoming.documents.map(d=>({title:String(d.title||'Course material'),url:String(d.url||''),type:String(d.type||'Google Slides')})).filter(d=>d.url):[],published:incoming.published!==false});guardianWriteJson(guardianTrainingFile,guardianTraining);guardianAdminAuditLog(req.guardianAdmin.username,'TRAINING_SAVED',{trainingId:t.id,title:t.title});res.json({ok:true,session:t})});
app.post('/api/admin/training/:id/discord',guardianRequireAdmin('settings.edit'),async(req,res)=>{const t=guardianTraining.find(x=>x.id===req.params.id);if(!t)return res.status(404).json({ok:false,error:'Training not found'});const channelId=String(req.body?.channelId||t.discordChannelId||guardianDiscord.briefingChannelId||guardianDiscord.patrolChannelId||'').trim();if(!channelId)return res.status(400).json({ok:false,error:'Choose a Discord channel'});const q=guardianQualifications.find(x=>x.id===t.qualificationId);const fields=[{name:'Service',value:(guardianServices.find(s=>s.id===t.serviceId)||{}).name||t.serviceId||'Multi-service',inline:true},{name:'Starts',value:t.startsAt||'TBC',inline:true},{name:'Location',value:t.location||'TBC',inline:true},{name:'Trainer',value:t.instructor||'TBC',inline:true},{name:'Spaces',value:t.maxSlots?`${(t.bookings||[]).length} / ${t.maxSlots}`:`${(t.bookings||[]).length} booked`,inline:true},{name:'Bookings close',value:t.bookingManuallyClosed?'Closed by staff':(t.bookingClosesAt||'TBC'),inline:true}];if(q)fields.push({name:'Qualification awarded',value:q.name,inline:false});if((t.prerequisites||[]).length)fields.push({name:'Prerequisites',value:(t.prerequisites||[]).map(id=>(guardianQualifications.find(q=>q.id===id)||{}).name||id).join(', ').slice(0,1024),inline:false});if((t.documents||[]).length)fields.push({name:'Course materials',value:t.documents.map(d=>`[${d.title}](${d.url})`).join('\n').slice(0,1024),inline:false});const payload={embeds:[guardianDiscordEmbed(`🎓 ${t.title}`,t.description||'Guardian training session.',{color:0x1e73be,fields})]};let r;if(t.discordMessageId&&String(t.discordChannelId||'')===channelId)r=await guardianDiscordRequest('PATCH',`/channels/${encodeURIComponent(channelId)}/messages/${encodeURIComponent(t.discordMessageId)}`,payload);else r=await guardianDiscordSend(channelId,payload);if(!r.ok)return res.status(400).json({ok:false,error:r.error||'Discord send failed'});if(r.data?.id)t.discordMessageId=String(r.data.id);t.discordChannelId=channelId;t.discordSentAt=new Date().toISOString();guardianWriteJson(guardianTrainingFile,guardianTraining);guardianAdminAuditLog(req.guardianAdmin.username,'TRAINING_SENT_DISCORD',{trainingId:t.id,channelId});res.json({ok:true,session:t})});
app.post('/api/admin/training/:id/bookings',guardianRequireAdmin('settings.edit'),(req,res)=>{const t=guardianTraining.find(x=>x.id===req.params.id);if(!t)return res.status(404).json({ok:false,error:'Training not found'});const username=String(req.body?.username||'');const action=String(req.body?.action||'add');if(!guardianAdminUsers.has(username))return res.status(404).json({ok:false,error:'User not found'});t.bookings=Array.isArray(t.bookings)?t.bookings:[];if(action==='remove')t.bookings=t.bookings.filter(x=>x!==username);else if(!t.bookings.includes(username))t.bookings.push(username);guardianWriteJson(guardianTrainingFile,guardianTraining);guardianAdminAuditLog(req.guardianAdmin.username,action==='remove'?'TRAINING_BOOKING_REMOVED':'TRAINING_BOOKING_ADDED',{trainingId:t.id,username});res.json({ok:true,session:t})});
app.post('/api/admin/training/:id/booking-state',guardianRequireAdmin('settings.edit'),(req,res)=>{const t=guardianTraining.find(x=>x.id===req.params.id);if(!t)return res.status(404).json({ok:false,error:'Training not found'});t.bookingManuallyClosed=req.body?.closed===true;guardianWriteJson(guardianTrainingFile,guardianTraining);guardianAdminAuditLog(req.guardianAdmin.username,t.bookingManuallyClosed?'TRAINING_BOOKINGS_CLOSED':'TRAINING_BOOKINGS_REOPENED',{trainingId:t.id});res.json({ok:true,session:t})});
app.post('/api/admin/training/:id/complete',guardianRequireAdmin('settings.edit'),async(req,res)=>{const t=guardianTraining.find(x=>x.id===req.params.id);if(!t)return res.status(404).json({ok:false,error:'Training not found'});const results=Array.isArray(req.body?.results)?req.body.results:(Array.isArray(req.body?.usernames)?req.body.usernames.map(username=>({username,status:'pass',note:''})):(t.bookings||[]).map(username=>({username,status:'pass',note:''})));t.results=Array.isArray(t.results)?t.results:[];for(const row of results){const username=String(row.username||''),status=String(row.status||'pass'),note=String(row.note||'');const u=guardianAdminUsers.get(username);if(!u)continue;t.results=t.results.filter(x=>x.username!==username);t.results.push({username,status,note,recordedBy:req.guardianAdmin.username,recordedAt:new Date().toISOString()});if(status==='pass'&&t.qualificationId){const q=guardianQualifications.find(x=>x.id===t.qualificationId);if(q){u.qualifications=Array.isArray(u.qualifications)?u.qualifications:[];u.qualifications=u.qualifications.filter(x=>(typeof x==='string'?x:x.id||x.name)!==q.id&&(typeof x==='string'?x:x.name)!==q.name);const months=Number(q.validityMonths||0),awardedAt=new Date(),expiresAt=months>0?new Date(new Date(awardedAt).setMonth(awardedAt.getMonth()+months)).toISOString():'';u.qualifications.push({id:q.id,name:q.name,serviceId:q.serviceId,awardedAt:awardedAt.toISOString(),awardedBy:req.guardianAdmin.username,expiresAt,discordRoleId:q.discordRoleId||''});await guardianQualificationDiscordRole(u,q,true)}}}guardianSaveUsers();t.completedAt=new Date().toISOString();t.bookingManuallyClosed=true;guardianWriteJson(guardianTrainingFile,guardianTraining);guardianAdminAuditLog(req.guardianAdmin.username,'TRAINING_COMPLETED',{trainingId:t.id,results:results.map(x=>({username:x.username,status:x.status}))});res.json({ok:true,session:t})});
