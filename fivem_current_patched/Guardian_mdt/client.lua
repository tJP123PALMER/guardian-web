...
-- Guardian standby moves use the same MDT alert/popup experience as a normal
-- mobilisation but remain a separate operational record.
RegisterNetEvent('guardian:standbyMove', function(move)
    local fake = {
        id = 'STANDBY:' .. tostring(move.id or ''),
        type = 'STANDBY MOVE',
        priority = 'Standby',
        address = tostring(move.destination or ''),
        postal = '',
        caller = 'CONTROL',
        notes = tostring(move.note or 'Proceed to standby station and acknowledge Control.'),
        details = tostring(move.note or ''),
        assignedUnits = { playerCallsign },
        playAlert = true,
        standbyMoveId = move.id,
        isStandbyMove = true
    }

    table.insert(incidentCache, fake)
    SendNUIMessage({ type = 'incident', item = fake })
    SendNUIMessage({ type = 'alert' })

    nuiOpen = true
    SetNuiFocus(true, true)
    SendNUIMessage({ type = 'open' })
    SendNUIMessage({ type = 'setCallsign', callsign = playerCallsign })
    SendNUIMessage({ type = 'mobilising' })
end)

RegisterNetEvent('guardian:standbyCancelled', function(move)
    SendNUIMessage({
        type = 'message',
        item = {
            sender = 'CONTROL',
            text = 'Standby move cancelled.',
            time = ''
        }
    })
end)

RegisterNetEvent('guardian:standbyReturn', function(move)
    SendNUIMessage({
        type = 'message',
        item = {
            sender = 'CONTROL',
            text = 'Return to home station.',
            time = ''
        }
    })
end)
