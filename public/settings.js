...
async function renderFormsBuilder(){
 ...
 const redraw=()=>{setContent(`<div class="card"><div class="sectionHead"><div><div class="eyebrow">DYNAMIC FORMS</div><h3>Form Builder</h3><p>Create forms that appear automatically in the member Forms tab.</p></div><button id="addDynamicForm">ADD FORM</button></div>${forms.map((f,fi)=>`<section class="opsEditor"><div class="three"><label>Title<input data-f-title="${fi}" value="${esc(f.title||'')}"></label><label>Category<input data-f-cat="${fi}" value="${esc(f.category||'Forms')}"></label><label>Audience<select data-f-aud="${fi}">${['guest','all','member','whitelisted','staff'].map(x=>`<option ${x===f.audience?'selected':''}>${x}</option>`).join('')}</select></label></div><label>Description<textarea data-f-desc="${fi}" rows="2">${esc(f.description||'')}</textarea></label><div class="toggleList"><label><input type="checkbox" data-f-pub="${fi}" ${f.published!==false?'checked':''}> Published</label><label><input type="checkbox" data-f-approval="${fi}" ${f.requiresApproval!==false?'checked':''}> Staff approval workflow</label></div><div class="sectionHead"><b>Fields</b><button data-f-addfield="${fi}">ADD FIELD</button></div>${(f.fields||[]).map((x,xi)=>`<div class="fieldRow"><input data-field-label="${fi}:${xi}" value="${esc(x.label||'')}" placeholder="Question / field label"><select data-field-type="${fi}:${xi}">${['text','textarea','select','checkbox','date','number'].map(t=>`<option ${t===x.type?'selected':''}>${t}</option>`).join('')}</select><label><input type="checkbox" data-field-req="${fi}:${xi}" ${x.required?'checked':''}> Required</label><button class="danger" data-field-del="${fi}:${xi}">×</button></div>`).join('')}<button class="danger" data-f-del="${fi}">DELETE FORM</button></section>`).join('')}<button id="saveForms">SAVE & PUBLISH FORMS</button></div><div class="card"><div class="eyebrow">SUBMISSIONS</div><h3>Submission Queue</h3><div class="tableWrap"><table><thead><tr><th>Form</th><th>Member</th><th>Status</th><th>Submitted</th><th>Action</th></tr></thead><tbody>${subs.map(s=>`<tr><td>${esc(s.formTitle)}</td><td>${esc(s.displayName||s.username)}</td><td><span class="statusBadge ${s.status==='approved'?'ok':'warn'}">${esc(s.status)}</span></td><td>${new Date(s.submittedAt).toLocaleString()}</td><td><button data-sub-review="${s.id}">UPDATE</button></td></tr>`).join('')||'<tr><td colspan="5">No submissions yet.</td></tr>'}</tbody></table></div></div>`);
 document.querySelectorAll('[data-f-del]').forEach(b=>b.onclick=()=>{forms.splice(+b.dataset.fDel,1);redraw()});
 document.querySelectorAll('[data-f-addfield]').forEach(b=>b.onclick=()=>{const fi=+b.dataset.fAddfield;forms[fi].fields=forms[fi].fields||[];forms[fi].fields.push({id:'field-'+Date.now(),type:'text',label:'New field',required:false});redraw()});
 document.querySelectorAll('[data-field-del]').forEach(b=>b.onclick=()=>{const [fi,xi]=b.dataset.fieldDel.split(':').map(Number);forms[fi].fields.splice(xi,1);redraw()});
 $('addDynamicForm').onclick=()=>{forms.push({id:'form-'+Date.now(),title:'New Form',description:'',category:'Forms',audience:'member',published:false,requiresApproval:true,fields:[]});redraw()};
 $('saveForms').onclick=async()=>{forms.forEach((f,fi)=>{f.title=document.querySelector(`[data-f-title="${fi}"]`).value.trim();f.category=document.querySelector(`[data-f-cat="${fi}"]`).value.trim();f.audience=document.querySelector(`[data-f-aud="${fi}"]`).value;f.description=document.querySelector(`[data-f-desc="${fi}"]`).value.trim();f.published=document.querySelector(`[data-f-pub="${fi}"]`).checked;f.requiresApproval=document.querySelector(`[data-f-approval="${fi}"]`).checked;(f.fields||[]).forEach((x,xi)=>{x.label=document.querySelector(`[data-field-label="${fi}:${xi}"]`).value.trim();x.type=document.querySelector(`[data-field-type="${fi}:${xi}"]`).value;x.required=document.querySelector(`[data-field-req="${fi}:${xi}"]`).checked;x.id=x.id||('field-'+fi+'-'+xi)})});await api('/api/admin/forms',{method:'POST',body:JSON.stringify({forms})});notify('Forms published');renderFormsBuilder()};
 document.querySelectorAll('[data-sub-review]').forEach(b=>b.onclick=async()=>{const status=prompt('Status: submitted, under-review, approved, rejected, closed','under-review');if(!status)return;const note=prompt('Staff note (optional)','')||'';await api('/api/admin/form-submissions/'+b.dataset.subReview+'/review',{method:'POST',body:JSON.stringify({status,note})});notify('Submission updated');renderFormsBuilder()});};redraw()
 }catch(e){errorView('Forms Builder',e)}
}
